...
export default function AccountRules({ 
    ...
    return (
      <div className="card" style={{ padding: '24px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '200px' }}>
          ...
          <span style={{ marginTop: '12px', color: 'var(--muted)', fontSize: 13 }}>Loading account rules…</span>
        </div>
      </div>
    );
  }
  ...
  const profitableDays = trades
    .filter(t => t.pnl > (cap * 0.001)) // 0.1% threshold
  ...
  return (
    <div className="card" style={{ padding: 0 }}>
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <span className="tag tag-blue" style={{ fontSize: 11, textTransform: 'uppercase' }}>
            {plan.plan_type}
          </span>
