'use client';
import { Settings2, Copy, ClipboardPaste, Trash2, Lock, LockOpen, EyeOff, ArrowUpToLine, ArrowDownToLine, Eraser } from 'lucide-react';
...
// menu never spills out of the canvas.
export default function ChartContextMenu({ x, y, id, locked, hidden, hasClipboard, bounds, onAction, onClose }) {
  const width = 184; const height = 246;
  const left = Math.max(4, Math.min(x, (bounds?.width || 800) - width - 4));
  const top = Math.max(4, Math.min(y, (bounds?.height || 440) - height - 4));
  const Item = ({ icon: Icon, label, danger = false, disabled = false, action }) => (
    <button
      disabled={disabled}
  ...
  return (
    <div style={{ position: 'absolute', left, top, width, zIndex: 120, background: 'var(--card2)', border: '1px solid var(--line2)', borderRadius: 10, boxShadow: '0 16px 40px rgba(0,0,0,.5)', overflow: 'hidden' }}>
      {id ? <>
