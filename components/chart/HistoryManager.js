...
export function createHistoryManager() {
  ...
  const commit = (entry) => { stack.splice(cursor + 1); stack.push(entry); cursor = stack.length - 1; };
  ...
  return {
    ...
    // Backward-compatible snapshot API for simple consumers.
    ...
    undo() { if (cursor < 0) return false; const entry = stack[cursor--]; [...entry.actions].reverse().forEach((action) => run(action, 'revert')); return true; },
    redo() { if (cursor >= stack.length - 1) return false; const entry = stack[++cursor]; entry.actions.forEach((action) => run(action, 'apply')); return true; },
    clear() { stack.length = 0; cursor = -1; group = null; },
  };
}
