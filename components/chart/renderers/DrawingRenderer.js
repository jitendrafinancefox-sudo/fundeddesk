'use client';
import { renderDrawing, isZoneType, isChannelType, isFibType, isStrokeType } from '../drawing/DrawingDefinitions';
export function DrawingRenderer({ drawings, transform, selectedId, selectedIds, hoverId }) {
  const selectedSet = selectedIds ? new Set(selectedIds) : null;
  return (ctx) => {
    drawings.forEach((drawing) => {
      if (isZoneType(drawing.drawingType) || isChannelType(drawing.drawingType) || isFibType(drawing.drawingType) || isStrokeType(drawing.drawingType)) return; // zones/channels/fib/strokes render on their own threads
